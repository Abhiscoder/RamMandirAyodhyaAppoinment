...
const MyAppointments = () => {
  ...
  const slotDateFormat = (slotDate) => {
    if (!slotDate || slotDate.length !== 7) return "Invalid Date";

    const day = slotDate.slice(0, 2);
    const monthIndex = Number(slotDate.slice(2, 3));
    const year = slotDate.slice(3);
  ...
  const appointmentRazorpay = async (appointmentId) => {
    ...
    try {
      ...
      if (data.success) {
        console.log("RazorPay Amount:", data.success)
        initPay(data.order)
      }
        ...
        {appointments.map((item, index) => (
          <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border' key={index}>
            ...
            <div className='flex-1 text-sm text-orange-400'>
              ...
              <b className='text-black'>Fee: Rs. <span>{item.docData.fees}</span></b>
            </div>

            <div></div>
            <div className='flex flex-col gap-2 justify-end'>
              {!item.cancelled && <button onClick={() => appointmentRazorpay(item._id)} className='text-sm text-center sm:min-w-48 py-2 border rounded bg-green-600 text-white hover:text-white hover:bg-orange-400 transition-all duration-300'>Pay Online</button>}
